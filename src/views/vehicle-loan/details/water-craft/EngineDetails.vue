...
const { data: years } = useGetYears("boats", "engines");

const getMakes = async (year: string) => {
  const res = await axiosThirdParty.get(`/vehicles/boats/engines/makes`, {
    params: { year: year },
  });
...
const getModel = async (params: unknown) => {
  const res = await axiosThirdParty.get(`/vehicles/boats/engines/models`, {
    params,
  });
...
const getVarient = async (params: unknown) => {
  const res = await axiosThirdParty.get(`/vehicles/boats/engines/variants`, {
    params,
  });
...
const getVehicle = async (params: unknown) => {
  const res = await axiosThirdParty.get(`/vehicles/boats/engines/nvic`, {
    params,
  });
    ...
    mutate(
      {
        ...
        engine: formValue.vehicle.modelName,
      },
      {
        onSuccess() {
